...
def GaussRadon_CPU(I, x, r, t, w0, w1, p0, p1, R):
    s0 = sqrt(2 * pi) * I
    for jj in range(w0.shape[0]):
        for k0 in range(p0.shape[0]):
            for k1 in range(p1.shape[0]):
                tmp = 0
                for ii in range(x.shape[0]):
                    rT = [r[ii, 0] * t[jj, 0] + r[ii, 3] * t[jj, 1] + r[ii, 5] * t[jj, 2],
                          r[ii, 1] * t[jj, 1] +
                          r[ii, 4] * t[jj, 2],
                          r[ii, 2] * t[jj, 2]]
                    rX = [p0[k0] * w0[jj, 0] + p1[k1] * w1[jj, 0] - x[ii, 0],
                          p0[k0] * w0[jj, 1] + p1[k1] *
                          w1[jj, 1] - x[ii, 1],
                          p0[k0] * w0[jj, 2] + p1[k1] * w1[jj, 2] - x[ii, 2]]
...
def GaussRadon_GPU(I, x, r, t, w0, w1, p0, p1, R):
    s0 = context().mul(I, sqrt(2 * pi))
    if BATCH:
        grid = w0.shape[0], p0.shape[0], p1.shape[0]
        tpb = 4
        __GaussRadon_GPU[tuple(-(-g // tpb) for g in grid),
                         (tpb, tpb, tpb)](s0, x, r, t, w0, w1, p0, p1, R)
    else:
        ...
        for i in range(w0.shape[0]):
            __GaussRadon_GPU[grid, (1, tpb, tpb)](
                s0, x, r, t[i:i + 1], w0[i:i + 1],
                w1[i:i + 1], p0, p1, R[i:i + 1])


@cuda.jit("void(f4[:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:])", cache=True)
def __GaussRadon_GPU(s0, x, r, t, w0, w1, p0, p1, R):
    jj, k0, k1 = cuda.grid(3)
    tmp = 0
    for ii in range(x.shape[0]):
        rT0 = r[ii, 0] * t[jj, 0] + r[ii, 3] * t[jj, 1] + r[ii, 5] * t[jj, 2]
        rT1 = r[ii, 1] * t[jj, 1] + r[ii, 4] * t[jj, 2]
        rT2 = r[ii, 2] * t[jj, 2]
        rX0 = p0[k0] * w0[jj, 0] + p1[k1] * w1[jj, 0] - x[ii, 0]
        rX1 = p0[k0] * w0[jj, 1] + p1[k1] * w1[jj, 1] - x[ii, 1]
        rX2 = p0[k0] * w0[jj, 2] + p1[k1] * w1[jj, 2] - x[ii, 2]
...
@numba.jit(["void(T[:],T[:,:],T[:,:],T[:,:],T[:,:],T[:,:],T[:],T[:],T[:,:,:],T[:])".replace('T', T) for T in ['f4', 'f8']], target='cpu', cache=True, nopython=True)
def GaussRadon_DI_CPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    s0 = sqrt(2 * pi)
    DR[:] = 0
    for jj in range(w0.shape[0]):
        for k0 in range(p0.shape[0]):
...
def GaussRadon_DI_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    s0 = sqrt(2 * pi)
    grid = x.shape[0]
    if BATCH:
        ...
        __GaussRadon_DI_GPU[grid, THREADS](
            s0, x, r, t, w0, w1, p0, p1, C, DR, array(sz, dtype='int32'))
    else:
        c = context()
        d = c.copy(DR)
        c.set(DR[:], 0)
        ...
        for i in range(w0.shape[0]):
            __GaussRadon_DI_GPU[grid, THREADS](
                s0, x, r, t[i:i + 1], w0[i:i + 1],
                w1[i:i + 1], p0, p1, C[i:i + 1], d, sz)
            c.add(DR, d, DR)


@cuda.jit("void(f4,f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:],f4[:],i4[:])", cache=True)
def __GaussRadon_DI_GPU(s0, x, r, t, w0, w1, p0, p1, C, DR, sz):
    ...
    for indx in range(sz[3] * cc, sz[3] * (cc + 1)):
        jj = indx // (sz[1] * sz[2])
        k0 = (indx // sz[2]) - sz[1] * jj
        k1 = indx - sz[2] * (k0 + sz[1] * jj)
        if jj < sz[0]:
            rT0 = r[ii, 0] * t[jj, 0] + r[ii, 3] * \
                t[jj, 1] + r[ii, 5] * t[jj, 2]
            rT1 = r[ii, 1] * t[jj, 1] + r[ii, 4] * t[jj, 2]
            rT2 = r[ii, 2] * t[jj, 2]
            rX0 = p0[k0] * w0[jj, 0] + p1[k1] * w1[jj, 0] - x[ii, 0]
            rX1 = p0[k0] * w0[jj, 1] + p1[k1] * w1[jj, 1] - x[ii, 1]
...
@numba.jit(["void(T[:],T[:,:],T[:,:],T[:,:],T[:,:],T[:,:],T[:],T[:],T[:,:,:],T[:,:])".replace('T', T) for T in ['f4', 'f8']], target='cpu', cache=True, nopython=True)
def GaussRadon_Dx_CPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    '''
...
def GaussRadon_Dx_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    s0 = context().mul(I, sqrt(2 * pi))
    grid = x.shape[0]
    if BATCH:
        sz = [w0.shape[0], p0.shape[0], p1.shape[0],
        ...
        for i in range(w0.shape[0]):
            __GaussRadon_Dx_GPU[grid, THREADS](
                s0, x, r, t[i:i + 1], w0[i:i + 1], w1[i:i + 1],
                p0, p1, C[i:i + 1], d, sz)
            c.add(DR, d, DR)


@cuda.jit("void(f4[:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:],f4[:,:],i4[:])", cache=True)
def __GaussRadon_Dx_GPU(s0, x, r, t, w0, w1, p0, p1, C, DR, sz):
...
@numba.jit(["void(T[:],T[:,:],T[:,:],T[:,:],T[:,:],T[:,:],T[:],T[:],T[:,:,:],T[:,:])".replace('T', T) for T in ['f4', 'f8']], target='cpu', cache=True, nopython=True)
def GaussRadon_Dr_CPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    s0 = sqrt(2 * pi) * I
    for ii in range(x.shape[0]):
        DR[ii, :] = 0
        for jj in range(w0.shape[0]):
            for k0 in range(p0.shape[0]):
                for k1 in range(p1.shape[0]):
                    rT = [r[ii, 0] * t[jj, 0] + r[ii, 3] * t[jj, 1] + r[ii, 5] * t[jj, 2],
                          ...
                          r[ii, 4] * t[jj, 2],
                          r[ii, 2] * t[jj, 2]]
                    X = [p0[k0] * w0[jj, 0] + p1[k1] * w1[jj, 0] - x[ii, 0],
                         p0[k0] * w0[jj, 1] + p1[k1] *
                         w1[jj, 1] - x[ii, 1],
                         p0[k0] * w0[jj, 2] + p1[k1] * w1[jj, 2] - x[ii, 2]]
                    ...
                    if inter > -20:
                        ...
                        # d/drij = s30*(rXi*tj + rTi*Xj) + s31*rTi*tj
                        # -rXi*Xj
                        ...
                        # d/dr22
                        DR[ii, 2] += inter * (s3[0] * (rX[2] * t[jj, 2] + rT[2] * X[2])
                                              + s3[1] * rT[2] * t[jj, 2] - rX[2] * X[2])
                        # d/dr01
                        DR[ii, 3] += inter * (s3[0] * (rX[0] * t[jj, 1] + rT[0] * X[1])
                                              + s3[1] * rT[0] * t[jj, 1] - rX[0] * X[1])
                        # d/dr12
                        DR[ii, 4] += inter * (s3[0] * (rX[1] * t[jj, 2] + rT[1] * X[2])
                                              + s3[1] * rT[1] * t[jj, 2] - rX[1] * X[2])
                        # d/dr02
                        DR[ii, 5] += inter * (s3[0] * (rX[0] * t[jj, 2] + rT[0] * X[2])
                                              + s3[1] * rT[0] * t[jj, 2] - rX[0] * X[2])


def GaussRadon_Dr_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    s0 = context().mul(I, sqrt(2 * pi))
    grid = x.shape[0]
    if BATCH:
        sz = [w0.shape[0], p0.shape[0], p1.shape[0],
...
@cuda.jit("void(f4[:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:],f4[:,:],i4[:])", cache=True)
def __GaussRadon_Dr_GPU(s0, x, r, t, w0, w1, p0, p1, C, DR, sz):
    ...
    for indx in range(sz[3] * cc, sz[3] * (cc + 1)):
        ...
        if jj < sz[0]:
            ...
            rT1 = r[ii, 1] * t[jj, 1] + r[ii, 4] * t[jj, 2]
            rT2 = r[ii, 2] * t[jj, 2]
            X0 = p0[k0] * w0[jj, 0] + p1[k1] * w1[jj, 0] - x[ii, 0]
            X1 = p0[k0] * w0[jj, 1] + p1[k1] * w1[jj, 1] - x[ii, 1]
            X2 = p0[k0] * w0[jj, 2] + p1[k1] * w1[jj, 2] - x[ii, 2]
            ...
            if inter > -20:
                ...
                # d/dr22
                sum2 += inter * (s30 * (rX2 * t[jj, 2] + rT2 * X2)
                                 + s31 * rT2 * t[jj, 2] - rX2 * X2)
                # d/dr01
                sum3 += inter * (s30 * (rX0 * t[jj, 1] + rT0 * X1)
                                 + s31 * rT0 * t[jj, 1] - rX0 * X1)
                # d/dr12
                sum4 += inter * (s30 * (rX1 * t[jj, 2] + rT1 * X2)
                                 + s31 * rT1 * t[jj, 2] - rX1 * X2)
                # d/dr02
                sum5 += inter * (s30 * (rX0 * t[jj, 2] + rT0 * X2)
                                 + s31 * rT0 * t[jj, 2] - rX0 * X2)
    buf[cc, 0] = sum0
    buf[cc, 1] = sum1
    buf[cc, 2] = sum2
    buf[cc, 3] = sum3
...
def GaussVol_CPU(I, x, r, y0, y1, y2, u):
    for jj in range(y0.shape[0]):
        for kk in range(y1.shape[0]):
            for ll in range(y2.shape[0]):
                tmp = 0
                for ii in range(x.shape[0]):
                    rX = [y0[jj] - x[ii, 0],
                          y1[kk] - x[ii, 1],
...
def GaussVol_GPU(I, x, r, y0, y1, y2, u):
    if BATCH:
        grid = y0.shape[0], y1.shape[0], y2.shape[0]
        tpb = 4
        __GaussVol_GPU[tuple(-(-g // tpb) for g in grid), (tpb, tpb, tpb)
                       ](I, x, r, y0, y1, y2, u)
    else:
        grid = 1, y1.shape[0], y2.shape[0]
        tpb = 4
        grid = tuple(-(-g // tpb) for g in grid)
        for i in range(y0.shape[0]):
            __GaussVol_GPU[grid, (1, tpb, tpb)
                           ](I, x, r, y0[i:i + 1], y1, y2, u[i:i + 1])


@cuda.jit("void(f4[:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:],f4[:,:,:])", cache=True)
def __GaussVol_GPU(I, x, r, y0, y1, y2, u):
    jj, kk, ll = cuda.grid(3)
    tmp = 0
    for ii in range(x.shape[0]):
        rX0 = y0[jj] - x[ii, 0]
        rX1 = y1[kk] - x[ii, 1]
        ...
        interior = -.5 * (rX0 * rX0 + rX1 * rX1 + rX2 * rX2)
        if interior > -20:
