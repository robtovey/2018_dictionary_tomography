...
@author: Rob Tovey
'''
from numpy import empty, pi, array, zeros, ascontiguousarray
import numba
...
def VolProj(atom, y, u):
    ...
    I, x, r = rebin(atom.I, atom.x, atom.r, y,
...
def __VolProj2(I, x, r, y0, y1, rad, u):
    ...
    y = cuda.local.array((2,), f4)
    for jjj in range(jj, min(jj + rad, y0.size)):
        y[0] = y0[jjj]
        for kkk in range(kk, min(kk + rad, y1.size)):
            y[1] = y1[kkk]
            tmp = 0
...
def __VolProj3(I, x, r, y0, y1, y2, rad, u):
    ...
    for jjj in range(jj, min(jj + rad, y0.size)):
        ...
        for kkk in range(kk, min(kk + rad, y1.size)):
            y[1] = y1[kkk]
            for lll in range(ll, min(ll + rad, y2.size)):
                y[2] = y2[lll]
                tmp = 0
                for ii in range(II.size):
                    if II[ii] == -1:
...
def RadProj(atom, Rad, R):
    ...
    if len(p) == 1:  # dim=2
        __RadProj_2D[tuple(-(-g // tpb) for g in grid),
                     (tpb,)](atom.I, atom.x, atom.r, t, w[0], p[0], R)
    else:
        if len(w) == 1:  # dim=2.5
            w = (w[0], empty((1, 1), dtype='f4'))
        __RadProj_3D[tuple(-(-g // tpb) for g in grid),
                     (tpb,)](atom.I, atom.x, atom.r, t, w[0], w[1], p[0], p[1], R)
...
def __rf3(I, x, r, y, T, W0, W1, s):
    ...
    limits[0, 0], limits[0, 1] = -s, s
    limits[1, 0], limits[1, 1] = -s, s
...
@cuda.jit(device=True, inline=True)
def __dRf2(x, r, y, T, rT, n, Y, rY, MY, R, dR, ddR, order):
    DIM = 2
    ...
    for i in range(DIM):
        ...
        dMydT[i, i] -= IP
    dg, dJdy, dJdT = cuda.local.array((2,), f4), cuda.local.array(
        (DIM,), f4), cuda.local.array((2,), f4)
    dg[0] = c_exp(-30 * m)
    dg[1] = -30 * dg[0]
    dg[2] = 900 * dg[0]
...
def __drf2(I, x, r, Y, T, W0, s, R, dR, ddR, order):
    ...
    limits[0] = X[0] * W0[0] + X[1] * W0[1] + X[2] * W0[2]
    limits[1] = min(limits[0] + rr, s)
    limits[0] = max(limits[0] - rr, -s)
     
    limits[0], limits[1] = -s, s
...
def derivs_RadProj(atom, Rad, C, order=2):
    ...
    if len(p) == 1:  # dim=2
        __derivs_RadProj_2D[block, THREADS](atom.I[0], atom.x[0], atom.r[0], t, w[0],
                                            p[0], C, f, Df, DDf, order)
    else:
        if len(w) == 1:  # dim=2.5
            w = (w[0], empty((1, 1), dtype='f4'))
        __derivs_RadProj_3D[block, THREADS](atom.I[0], atom.x[0], atom.r[0], t, w[0], w[1],
                                            p[0], p[1], C, f, Df, DDf, order)
  
    return f.sum(axis=-1), Df.sum(axis=-1), DDf.sum(axis=-1)
 
  
@cuda.jit
def __derivs_RadProj_2D(I, x, r, t, w0, w1, p0, p1, C, f, Df, DDf, order):
    '''
...
if __name__ == '__main__':
    ...
    Rad = dict2obj(ProjectionSpace=dict2obj(orientations=t, ortho=w, detector=p[:dim - 1]))
...
#     print(.5 * (myR ** 2).sum())
    ...
    T = tic()
    R, dR, ddR = derivs_RadProj(atom, Rad, 0 * myR, order=2)
