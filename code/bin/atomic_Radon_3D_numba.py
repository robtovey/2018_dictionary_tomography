        ...
        for k0 in range(p0.shape[0]):
            for k1 in range(p1.shape[0]):
                ...
                for ii in range(x.shape[0]):
                    IP = [x[ii, 0] * w0[jj, 0] + x[ii, 1] * w0[jj, 1] + x[ii, 2] * w0[jj, 2],
                          x[ii, 0] * w1[jj, 0] + x[ii, 1] * w1[jj, 1] + x[ii, 2] * w1[jj, 2]]
...
def GaussRadon_GPU(I, x, r, t, w0, w1, p0, p1, R):
    ...
    if BATCH:
        ...
        __GaussRadon_GPU[tuple(-(-g // tpb) for g in grid),
                         (tpb, tpb, tpb)](x, s1, s2, w0, w1, p0, p1, R)
    else:
...
def __GaussRadon_GPU(x, s1, s2, w0, w1, p0, p1, R):
    jj, k0, k1 = cuda.grid(3)
    if jj >= w0.shape[0] or k0 >= p0.shape[0] or k1 >= p1.shape[0]:
        return
    tmp = 0
    for ii in range(x.shape[0]):
        IP0 = p0[k0] - (x[ii, 0] * w0[jj, 0] + x[ii, 1] *
                        w0[jj, 1] + x[ii, 2] * w0[jj, 2])
        IP1 = p1[k1] - (x[ii, 0] * w1[jj, 0] + x[ii, 1] *
                        w1[jj, 1] + x[ii, 2] * w1[jj, 2])
...
def GaussRadon_DI_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    ...
    if BATCH:
        ...
        __GaussRadon_DI_GPU[grid, THREADS](
            x, s1, s2, w0, w1, p0, p1, C, DR, array(sz, dtype='int32'))
    else:
        c = context()
        d = c.copy(DR)
        c.set(DR[:], 0)
...
def __GaussRadon_DI_GPU(x, s1, s2, w0, w1, p0, p1, C, DR, sz):
...
def GaussRadon_Dx_CPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    ...
    for jj in range(w0.shape[0]):
        for k0 in range(p0.shape[0]):
            for k1 in range(p1.shape[0]):
                for ii in range(x.shape[0]):
                    tmp = [x[ii, 0] * w0[jj, 0] + x[ii, 1] * w0[jj, 1] + x[ii, 2] * w0[jj, 2],
                           x[ii, 0] * w1[jj, 0] + x[ii, 1] * w1[jj, 1] + x[ii, 2] * w1[jj, 2]]
                    IP = (p0[k0] - tmp[0], p1[k1] - tmp[1])
                    inter = (IP[0]**2 + IP[1]**2) * s2[ii]
                    if inter > -20:
                        inter = s1[ii] * exp(inter) * C[jj, k0, k1]

                        DR[ii, 0] += inter * \
                            (IP[0] * w0[jj, 0] + IP[1] * w1[jj, 0])
                        DR[ii, 1] += inter * \
                            (IP[0] * w0[jj, 1] + IP[1] * w1[jj, 1])
                        DR[ii, 2] += inter * \
                            (IP[0] * w0[jj, 2] + IP[1] * w1[jj, 2])


def GaussRadon_Dx_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    c = context()
    s1 = c.div(c.mul(I, sqrt(2 * pi)), r[:, 0])
    s2 = c.div(-1 / 2, c.mul(r[:, 0], r[:, 0]))
    grid = x.shape[0]
    if BATCH:
        sz = [w0.shape[0], p0.shape[0], p1.shape[0],
        ...
        for i in range(w0.shape[0]):
            __GaussRadon_Dx_GPU[grid, THREADS](
                x, s1, s2, w0[i:i + 1], w1[i:i + 1],
                p0, p1, C[i:i + 1], d, sz)
            c.add(DR, d, DR)


@cuda.jit("void(f4[:,:],f4[:],f4[:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:],f4[:,:],i4[:])", cache=True)
def __GaussRadon_Dx_GPU(x, s1, s2, w0, w1, p0, p1, C, DR, sz):
    ...
    for indx in range(sz[3] * cc, sz[3] * (cc + 1)):
        ...
        if jj < sz[0]:
            ...
            if inter > -20:
                ...
                sum0 += inter * (IP0 * w0[jj, 0] + IP1 * w1[jj, 0])
                sum1 += inter * (IP0 * w0[jj, 1] + IP1 * w1[jj, 1])
                sum2 += inter * (IP0 * w0[jj, 2] + IP1 * w1[jj, 2])
...
def GaussRadon_Dr_GPU(I, x, r, t, w0, w1, p0, p1, C, DR):
    ...
    if BATCH:
        ...
        if sz[3] * THREADS < sz[0] * sz[1] * sz[2]:
            sz[3] += 1
#         for thing in [x, s1, s2, w0, w1, p0, p1, C, DR]:
#             print(thing.shape)
#         exit()
...
@cuda.jit("void(f4[:,:],f4[:],f4[:],f4[:,:],f4[:,:],f4[:],f4[:],f4[:,:,:],f4[:],i4[:])", cache=True)
def __GaussRadon_Dr_GPU(x, s1, s2, w0, w1, p0, p1, C, DR, sz):
    buf = cuda.shared.array(THREADS, dtype=numba.float32)
    ii = cuda.blockIdx.x
    cc = cuda.threadIdx.x
...
#             inter = (IP0**2 + IP1**2) * s2[ii]
#             if inter > -20:
#                 sum0 += s1[ii] * c_exp(inter) * \
#                     C[jj, k0, k1] * (0.5 - inter)
    buf[cc] = sum0
    cuda.syncthreads()
    ...
    if BATCH:
        ...
        for i in range(y0.shape[0]):
            __GaussVol_GPU[grid, (1, tpb, tpb)
                           ](I, x, s, y0[i:i + 1], y1, y2, u[i:i + 1])


@cuda.jit("void(f4[:],f4[:,:],f4[:],f4[:],f4[:],f4[:],f4[:,:,:])", cache=True)
def __GaussVol_GPU(I, x, s, y0, y1, y2, u):
